...
def render_map():
    ...
    for c in contatori:
        markers += "L.marker(["+str(c[1])+", "+str(c[2]) + \
            ...
            "Icon}).bindPopup('<div class=\"container-fluid\" style=\"text-align:center;font-size:20px\"><p style=\"font-weight:1000\">"+str(int(c[0]))+"</p><p><a href=\"https://prova1-370722.ew.r.appspot.com/graph/temp/" + \
            str(int(c[0])) + \
...
def read_data_temp(metercode):
    ...
    for doc in db.collection("lista contatori").stream():
        if metercode == str(doc.id):
            ...
            lista_contatori = lista_contatori + \
                "<a href=\"https://prova1-370722.ew.r.appspot.com/graph/temp/"+doc.id+"\"1 style=\"text-decoration:none\"><p style=\"color:rgb(10, 12, 150);font-size:20px;font-weight:800;background-color:white;\">"+str(
                    doc.id)+"</p></a>"
...
def read_data_port(metercode):
    ...
    for doc in db.collection("lista contatori").stream():
        if metercode == str(doc.id):
            ...
            lista_contatori = lista_contatori + \
                "<a href=\"https://prova1-370722.ew.r.appspot.com/graph/port/"+doc.id+"\"1 style=\"text-decoration:none\"><p style=\"color:rgb(10, 12, 150);font-size:20px;font-weight:800;background-color:white;\">"+str(
                    doc.id)+"</p></a>"
